...
const SHAPE_TYPE_LIST: ElementType[] = [
  "mandatory-vp",
  "optional-vp",
  "variant",
];
...
export function Canvas() {
    ...
    if (action === "drawing") {
      ...
      if (isLineType(lastEl.type)) {
        ...
        if (snap) {
          updateElement(lastEl.id, {
            x2: snap.snappedPoint.x,
            y2: snap.snappedPoint.y,
          });
        ...
        if (movedShapeIds.size > 0) {
          newElements = newElements.map((el) => {
            ...
            if (
              ...
              const shape = newElements.find(
                (s) => s.id === el.startBinding!.elementId,
              )!;
            ...
            if (el.endBinding && movedShapeIds.has(el.endBinding.elementId)) {
              const shape = newElements.find(
                (s) => s.id === el.endBinding!.elementId,
              )!;
      ...
      if (snap) {
        updateElement(selectedElementId, {
          x1: snap.snappedPoint.x,
          y1: snap.snappedPoint.y,
        });
        pendingStartBinding.current = snap.binding;
        setSnapIndicator({ x: snap.snappedPoint.x, y: snap.snappedPoint.y });
      } else {
  ...
  const handleDoubleClick = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    if (element) {
      if (
        SHAPE_TYPE_LIST.includes(element.type) ||
        element.type === "alternative-arc"
      ) {
  ...
  const handleExport = useCallback(() => {
    ...
    svgClone
      .querySelectorAll('rect[fill="url(#dotGrid)"]')
      .forEach((r) => r.remove());
          ...
          if (selectedElementIds.size > 0) {
            setElements((prev) => {
              ...
              return filtered.map((el) => {
                ...
                if (
                  el.startBinding &&
                  selectedElementIds.has(el.startBinding.elementId)
                ) {
                  updated = { ...updated, startBinding: undefined };
                }
                if (
                  el.endBinding &&
                  selectedElementIds.has(el.endBinding.elementId)
                ) {
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        setCamera((prev) => {
          const newZoom = Math.min(
            Math.max(0.1, prev.zoom * (1 + zoomDelta)),
            5,
          );
  ...
  const isDrawingLine =
    (action === "drawing" &&
      elements.length > 0 &&
      isLineType(elements[elements.length - 1].type)) ||
